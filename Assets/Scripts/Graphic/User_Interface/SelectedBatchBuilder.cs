...
/// 매 프레임 selectedBlockList를 순회하여 마우스를 따라다니는 반투명 블록의 배치 데이터를 구성합니다.
/// </summary>
public class SelectedBatchBuilder : MonoBehaviour
{
    ...
    /// <summary>
    /// 매 프레임 selectedBlockList를 순회하여 렌더링 배치 데이터를 재구성합니다.
    /// </summary>
    public void RunAfterFrame()
    {
        ...
        var selectedList = game.Player.selectedBlockList;
        ...
        _keyBuffer.Clear();
        foreach (var key in selectedList.Keys) {
            _keyBuffer.Add(key);
        }

        for (int i = 0; i < _keyBuffer.Count; ++i) {
            int dictKey = _keyBuffer[i];
            SelectedBlock selected = selectedList[dictKey];
            if (selected.IsVoid())
                continue;

            EBlock id = selected.id;
            SO_Block so = GetSO(id);
            if (so == null)
                continue;

            float baseX = cursor.x + selected.offset.x;
            float baseY = cursor.y + selected.offset.y;

            // 블록 설치 방향 각도 (모든 부품의 오프셋을 이 각도로 회전)
            float blockAngle = UGraphic.RotateToAngle(selected.rotate);
            ...
            for (int si = 0; si < spriteCount; ++si) {
                ...
                float posZ = -10f + info.offset.z;

                // Selected 상태에서는 모든 부품이 블록 방향으로만 회전
                Quaternion rot;
                switch (info.type) {
                ...
                // 오프셋도 블록 방향으로 회전, 셀 중심으로 보정
                Vector3 pos = UGraphic.RotateOffset(baseX + 0.5f, baseY + 0.5f, info.offset.x, info.offset.y, blockAngle, posZ);
